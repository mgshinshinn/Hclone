...
export default function App() {
  ...
  const handleGenerate = async () => {
    setError(null);
    if (!srtInput.trim()) {
      setError("Please paste some text or upload an SRT file.");
      return;
    }
    ...
    try {
      const textToProcess = srtInput.trim();
      if (!textToProcess) throw new Error("No readable text found in input.");
      ...
      try {
        ...
        throw new Error("Server took too long or returned an invalid response. Please try with a shorter text (e.g. 3-5 minutes).");
      }
      ...
      if (data.speedAdjustments) {
        setSpeedAdjustments(data.speedAdjustments);
      }
      if (data.speedAdjustmentsWithEnd) {
        setSpeedAdjustmentsWithEnd(data.speedAdjustmentsWithEnd);
      }
    } catch (err: any) {
...
print(f"✅ Success! Video processing complete. Download '{output_video}' from the folder menu.")`;
  ...
  const generateColabScriptRemoveAllGaps = () => {
    const defaultName = fileName ? fileName.trim() : "video";
    const inputVideo = `${defaultName}.mp4`;
    const outputVideo = `${defaultName}_Edit.mp4`;
    const speedText = speedAdjustments.join("\n");
...
print(f"✅ Success! Video processing complete. Download '{output_video}' from the folder menu.")`;
  ...
  const generateColabScriptRemoveAllGapsFlipAntiCopyright = () => {
    const defaultName = fileName ? fileName.trim() : "video";
    const inputVideo = `${defaultName}.mp4`;
    const outputVideo = `${defaultName}_Edit.mp4`;
    const speedText = speedAdjustments.join("\\n");
...
for i, seg in enumerate(segments):
    ...
    # Periodical flip logic (random between 7 and 12 seconds)
...
# Anti-Copyright Zoom & Color filter (scale up, crop down to simulate zoom, adjust EQ)
...
filter_script += f"{concat_inputs}concat=n={len(segments)}:v=1:a=1[conc_v][conc_a];\\n"

# Anti-Copyright Zoom & Color filter string
filter_script += "[conc_v]scale=w='trunc(iw*1.04/2)*2':h='trunc(ih*1.04/2)*2',crop=w='trunc(iw/1.04/2)*2':h='trunc(ih/1.04/2)*2',eq=brightness=0.03:contrast=1.03:saturation=1.05[outv];\\n"
filter_script += "[conc_a]anull[outa]"
          ...
          <div className="md:col-span-8 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="relative">
                  <input
                    ...
                    id="srt-upload"
                  />
                  <label htmlFor="srt-upload" className="flex items-center gap-2 text-sm font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-md cursor-pointer transition">
              ...
              <textarea
                ...
                placeholder="Paste your SRT or text here, or upload a file..."
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <div className="text-xs text-slate-400">
                  <span>{srtInput.length} chars</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                  ...
                  {isSettingsOpen && (
                    ...
                    >
                      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-5 mb-4">
                        ...
                        {/\d{2}:\d{2}:\d{2},\d{3}\s*-->\s*\d{2}:\d{2}:\d{2},\d{3}/.test(srtInput) && (
                          <div>
                            <label className="block text-sm font-medium text-slate-700 mb-2">
                              Speed Adjustment
                            </label>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100">
                <button
                  ...
                  disabled={isGenerating || !srtInput.trim()}
                    ...
                    <div className="mt-6 w-full text-left space-y-6">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <button
                            onClick={() => navigator.clipboard.writeText(generateColabScriptRemoveAllGaps())}
                            className="flex items-center gap-1.5 text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-md transition"
                          >
                            <Copy className="w-3.5 h-3.5" />
                            Copy
                        ...
                        <textarea
                          readOnly
                          value={generateColabScriptRemoveAllGaps()}
                          className="w-full bg-white border border-slate-200 rounded-lg p-3 h-40 text-xs font-mono text-blue-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 resize-none"
                        />
                      </div>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <label className="text-sm font-semibold text-slate-800">
                            Remove All Gaps (Flip & Anti-Copyright)
                          </label>
                          <button
                            onClick={() => navigator.clipboard.writeText(generateColabScriptRemoveAllGapsFlipAntiCopyright())}
                            className="flex items-center gap-1.5 text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-md transition"
                          >
                            <Copy className="w-3.5 h-3.5" />
                            Copy
                        ...
                        <textarea
                          readOnly
                          value={generateColabScriptRemoveAllGapsFlipAntiCopyright()}
                          className="w-full bg-white border border-slate-200 rounded-lg p-3 h-40 text-xs font-mono text-blue-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 resize-none"
                        />
                      </div>
                      <div>
